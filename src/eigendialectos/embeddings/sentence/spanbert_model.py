"""SpanBERTa sentence embedding model.

Fine-tunes SpanBERTa (``chrisk/SpanBERTa``) -- a RoBERTa model
pre-trained on a large Spanish corpus -- and extracts sentence
embeddings via [CLS] pooling or mean pooling.
"""
...
def _require_transformers() -> None:
    if not _HAS_TRANSFORMERS:
        raise ImportError(
            "transformers and torch are required for SpanBERTaModel. "
            "Install with:  pip install transformers torch  "
            "or:  pip install eigendialectos[sentence]"
        )


_DEFAULT_MODEL = "chrisk/SpanBERTa"


class SpanBERTaModel(EmbeddingModel):
    """SpanBERTa sentence embedding model.
    ...
    device:
        PyTorch device string.  ``None`` for automatic selection.
    """
    ...
    def train(self, corpus: CorpusSlice, config: dict[str, Any] | None = None) -> None:
        """Fine-tune SpanBERTa on the dialect corpus using MLM."""
        _require_transformers()
        cfg = config or {}
        ...
        output_dir = cfg.get("output_dir", "/tmp/spanberta_finetune")

        logger.info(
            "Fine-tuning SpanBERTa (%s) for %d epochs on %d texts (%s)",
    ...
    def save(self, path: Path) -> None:
        ...
        self._save_meta(p)
        logger.info("SpanBERTa model saved to %s", p)
    ...
    def load(self, path: Path) -> None:
        ...
        self._is_trained = True
        logger.info("SpanBERTa model loaded from %s", p)
