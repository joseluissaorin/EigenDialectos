"""MarIA sentence embedding model.

Fine-tunes ``PlanTL-GOB-ES/roberta-base-bne`` (MarIA) and extracts
...
def _require_transformers() -> None:
    if not _HAS_TRANSFORMERS:
        raise ImportError(
            "transformers and torch are required for MarIAModel. "
            "Install with:  pip install transformers torch  "
            "or:  pip install eigendialectos[sentence]"
        )


_DEFAULT_MODEL = "PlanTL-GOB-ES/roberta-base-bne"


class MarIAModel(EmbeddingModel):
    """MarIA (RoBERTa-BNE) sentence embedding model.
    ...
    def train(self, corpus: CorpusSlice, config: dict[str, Any] | None = None) -> None:
        """Fine-tune MarIA on the dialect corpus using MLM."""
        _require_transformers()
        cfg = config or {}
        ...
        output_dir = cfg.get("output_dir", "/tmp/maria_finetune")

        logger.info(
            "Fine-tuning MarIA (%s) for %d epochs on %d texts (%s)",
    ...
    def save(self, path: Path) -> None:
        ...
        self._save_meta(p)
        logger.info("MarIA model saved to %s", p)
    ...
    def load(self, path: Path) -> None:
        ...
        self._is_trained = True
        logger.info("MarIA model loaded from %s", p)
